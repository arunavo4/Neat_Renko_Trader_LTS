...
logger = setup_logger('reporter', 'report.log')
...
class LoggerReporter(BaseReporter):
    ...
    def end_generation(self, config, population, species_set):
        ...
        logger.info('Total extinctions: {0:d}'.format(self.num_extinctions))
        if len(self.generation_times) > 1:
            logger.info("Generation time: {0:.3f} sec ({1:.3f} average)".format(elapsed, average))
        else:
            logger.info("Generation time: {0:.3f} sec".format(elapsed))
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        logger.info('Population\'s average fitness: {0:3.5f} stdev: {1:3.5f}'.format(fit_mean, fit_std))
        logger.info(
    ...
    def complete_extinction(self):
        ...
        logger.info('All species extinct.')
